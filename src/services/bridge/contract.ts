...
import stargateRefuelAbi from "./abi/stargateRefuel.json";
import request from "@/utils/request";
import { EVMConfig, IS_MAINNET } from "@/config/bridge";
...
import { getAuroraConfig } from "@/utils/auroraConfig";
import {
  uniswapV2Router02ContractAddress,
  uniswapV2Router02SwapPath,
  BridgeConfig,
} from "@/config/bridge";
import { UniswapRouterAbi } from "@/services/abi/IUniswapV2Router02";
import {
...
export const evmServices = {
  ...
  async getBalance(
    ...
    try {
      ...
      // console.error(error);
      logger.log(token.symbol, token.address);
      return "0";
    }
  },
...
export const uniswapV2Router02Services = {
  async getUniswapContract() {
    const provider = evmServices.getEvmJSONProvider("Aurora");
  ...
  async getAmountsOut(rawAmountIn) {
    const uniswapContract =
      await uniswapV2Router02Services.getUniswapContract();
    const amountOutMin = await uniswapContract.getAmountsOut(
      rawAmountIn,
      uniswapV2Router02SwapPath
    );
    return amountOutMin[amountOutMin.length - 1].toString();
  },
};
export const refuelServices = {
  async getRefuelContract() {
    const provider = evmServices.getEvmJSONProvider("Aurora");
    const contract = new ethers.Contract(
  ...
  async getSwapfee() {
    const RefuelReceiveContract = await refuelServices.getRefuelContract();
    const swapFee = await RefuelReceiveContract.swapFee();
    return swapFee.toString();
  },
  async getBridgefee() {
    const RefuelReceiveContract = await refuelServices.getRefuelContract();
    const bridgeFee = await RefuelReceiveContract.bridgeFee();
    return bridgeFee.toString();
  },
};
...
export const nearServices = {
  ...
  async sendTransaction(params: TransactionParams) {
    ...
    if (!transformedRes || wallet.id === "my-near-wallet") await sleep(5000);
  ...
  async checkFTStorageBalance(FTContractId: string, accountId?: string) {
    ...
    const res: any = await nearServices.query<{
      available: string;
      total: string;
    }>({
  ...
  async getBalance(tokenAddress: string, decimals?: number) {
    try {
      ...
      if (address === getTokenAddress("NEAR")) {
        ...
        if (Big(balance || 0).lt(0)) return "0";
      } else {
        balance =
...
export const tokenServices = {
  ...
  async getBalance(
    ...
    const res = await (chain === "NEAR"
      ? nearServices.getBalance(token.address, token.decimals)
