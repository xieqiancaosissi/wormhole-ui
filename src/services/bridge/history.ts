...
// import { formatNumber } from "@/services/limit/limitUtils";
...
const bridgeHistoryService = {
  async query(params: QueryParams) {
    ...
    data.list?.forEach((v) => {
      ...
      if (Big(v.to_usdc).gt(0)) {
        v.to_usdc = formatNumber(formatAmount(v.to_usdc, 6), {
          displayDecimals: 6,
        });
      } else {
        v.to_usdc = null;
      }
      if (Big(v.to_near).gt(0)) {
        v.to_near = formatNumber(formatAmount(v.to_near, 24), {
          displayDecimals: 6,
        });
      } else {
        v.to_near = null;
