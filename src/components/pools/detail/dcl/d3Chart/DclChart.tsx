...
import {
  ...
  beautifyPrice,
  beautifyPriceWithE,
} from "@/components/common/beautifyNumber";
...
function DclChart({
  ...
  const svgPaddingX = +(appearanceConfig.svgPaddingX || 12);
  ...
  function draw_axis({ scale }: { scale: any }) {
    ...
    axis.ticks(appearanceConfig.ticks || 5).tickFormat(function (d: any) {
      ...
      } else if (dBig.gte(0.0001) && dBig.lte(10000)) {
        return formatWithCommas(dBig.toFixed());
      } else {
        return beautifyPrice(dBig.toFixed());
      }
    });
  ...
  function get_current_price_by_token_y() {
    if (pool) {
      return beautifyNumber({ num: reverse_price(get_current_price()) });
    }
    return "-";
  }
  function zoomOut() {
      ...
      >
        ...
        <div className="overBox lg:w-75 xsm:w-full lg:min-h-48 flex flex-col absolute xsm:top-60 rounded-xl bg-modalGrayBg xsm:hidden  p-4  xsm:mt-4 z-10 lg:invisible">
          ...
          <div className="flex items-center justify-between my-2">
            ...
            <span className="text-xs text-white paceGrotesk-Bold flex items-center flex-wrap justify-end">
              {reverse ? (
                <>
                  {binDetail?.price_by_token_y
                    ? beautifyPriceWithE(Number(binDetail.price_by_token_y))
                    : "-"}{" "}
                  {pool?.token_x_metadata?.symbol} /{" "}
                  {binDetail?.price_by_token_x
                    ? beautifyPriceWithE(Number(binDetail.price_by_token_x))
                    : "-"}{" "}
        ...
        <div className="currentLine flex flex-col items-center absolute left-0 pointer-events-none z-0">
          ...
          ></div>
          <div
            className={`currentLineDetail  top-0 left-0 min-w-43 h-15 rounded-lg p-2.5 absolute transform -translate-x-1/2 -translate-y-full flex flex-col justify-between bg-modalGrayBg ${
