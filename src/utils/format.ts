...
import { EVMConfig, NearConfig } from "@/config/bridge";
...
export function formatTxExplorerUrl(
  ...
  if (onWormhole) {
    return `https://wormholescan.io/#/tx/${hash}?network=Mainnet`;
  } else if (EVMConfig.chains[_chain]) {
...
export function formatStaticFileUrl(key: string) {
  const AssetsCDN = "https://img.ref.finance";
  return `${AssetsCDN}/images${key}`;
}
